...
nodes <- data.frame(id = 1:7,
                    label = c("MPC::flow coupler", "PK::flow", "PK::overland flow", "MPC::flow and transport", "MPC::transport coupler", "PK::subsurface transport", "PK::surface transport"),  # Set node labels
...
edges <- data.frame(from = c(1, 1, 5, 5, 4, 4), to = c(2, 3, 6, 7, 1, 5))  # Connect "MPC::flow coupler" to "PK::flow" and "PK::overland flow"

# Define custom positions for the nodes (flip y-positions so MPC is above)
nodes$x <- c(0, -100, 100, 225, 450, 350, 600)  # Set the x positions (center, left, right)
nodes$y <- c(-100, 0, 0, -200, -100, 0, 0)  # Set the y positions (MPC at the top, PK nodes below)
...
saveWidget(network, file = "../dependency_transport.html", selfcontained = FALSE)
