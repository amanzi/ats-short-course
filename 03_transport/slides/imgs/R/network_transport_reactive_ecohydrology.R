...
nodes <- data.frame(id = 1:14,
                    ...
                    "MPC::flow and transport", "MPC::transport coupler", "PK::subsurface transport",
                     ...
                     "PK::chemistry surface", "PK::chemistry subsurface", "MPC::water balance",
                     "PK::canopy", "PK::snow"),  # Set node labels
                    ...
                    title = c("Strong coupling (PK Type: coupled water)",
                              ...
                              "Chemistry Alquimia (PK Type: chemistry alquimia)",
                              "Weak coupling (PK Type: weak MPC)",
                              "Weak coupling (PK Type: general surface balance)",
                              "Weak coupling (PK Type: general surface balance)"
                    ),
...
edges <- data.frame(from = c(1, 1, 5, 5, 9, 9, 8, 8, 4, 4, 12, 12), to = c(2, 3, 6, 7, 10, 11, 5, 9, 1, 8, 13, 14))  # Connect "MPC::flow coupler" to "PK::flow" and "PK::overland flow"

# Define custom positions for the nodes (flip y-positions so MPC is above)
nodes$x <- c(-200, -300, -100, 225, 360, 240, 500, 600, 850, 750, 1000, 0, 0, 150)  # Set the x positions (center, left, right)
nodes$y <- c(0, 100, 100, -200, 0, 100, 100, -100, 0, 100, 100, -100, 0, 0)  # Set the y positions (MPC at the top, PK nodes below)
...
saveWidget(network, file = "../dependency_transport_reactive_ecohydrology.html", selfcontained = FALSE)
